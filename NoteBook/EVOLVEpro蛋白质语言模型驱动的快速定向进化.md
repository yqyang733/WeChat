...
| **研究方向**       | **文章类型**       | **研究体系**       | **研究策略**       | **一句话内容简介**                                                                 |
|--------------------|--------------------|--------------------|--------------------|------------------------------------------------------------------------------------|
| 蛋白质定向进化      | 方法-实例验证      | 蛋白质语言模型；六种治疗相关蛋白质  | 少量样本主动学习    | EVOLVEpro通过少量样本和PLMs实现蛋白质活性等多种性质的多目标定向进化，显著提升结合、催化等特性。         |
...
T7 RNA 聚合酶（T7 RNAP） 在 mRNA 治疗、疫苗、细胞工程和基础科学研究中具有重要作用。与基因组编辑不同，mRNA 生产的效力和质量由多个特征决定，因此设计了多目标优化函数，重点关注以下三个参数：**RNA 产量（通过紫外-可见分光光度法测量）；mRNA 翻译效率（通过双链 RNA（dsRNA）敏感细胞系中的荧光素酶翻译测量）；RNA 纯度（通过 BJ 成纤维细胞中的干扰素 β（IFN-β）RNA 产生测量免疫原性）。** 在 EVOLVEpro 的目标函数中，这三个参数的权重分别为 20%、40% 和 40%，以优先考虑高保真和低免疫原性（图 5A）。  

**进化结果与实验验证。** 在前两轮进化中，顶级突变体的活性提高了 2 到 4 倍。在第 3 和第 4 轮中，观察到所有特征的显著改善，尤其是翻译效率和免疫原性（图 5B-C）。第 4 轮结束时，突变体 E643G 生成的荧光素酶 mRNA 翻译效率提高了 34 倍，免疫原性降低了 98%（图 5C）。在临床相关的体外转录（IVT）条件下，比较了 epT7、T7E643G 和野生型 T7 RNAP 的性能。结果显示，epT7 和 E643G 在 2 小时转录方案中生成的 mRNA 产量显著高于野生型和 G47A/884insG 突变体。  

**进化轨迹与全局分析。** T7 RNAP 的 活性 和 适应度（pMMS） 之间呈弱负相关（相关系数 -0.13），表明两者无显著关联（图 5H）。EVOLVEpro 通过选择高活性但不一定高适应度的突变体，成功导航了这种分歧。全局进化图谱显示，EVOLVEpro 学习的活性图谱与 ESM-2 预测的适应度图谱存在显著差异，进一步表明适应度预测无法完全预测蛋白质活性的改进（图 5I-K）。  

![](EVOLVEpro蛋白质语言模型驱动的快速定向进化/EVOLVEpro蛋白质语言模型驱动的快速定向进化_2025-02-04-11-37-21.png)  

### 使用 epT7（EVOLVEpro 增强型 T7 RNA 聚合酶）进行 环状 RNA 产量的研究
环状 RNA（circRNA） 因其增强的稳定性和延长的蛋白质表达，成为蛋白质替代治疗的有前景的治疗方式。由于 epT7 在转录中表现出显著降低的 dsRNA 生成和更高的保真度，假设 epT7 可以增强 circRNA 的生产，因为 RNase R 在体外转录（IVT）后处理中通常会富集 circRNA 和免疫原性的 dsRNA（图 6A）。  

**circRNA 生产效率与翻译能力。** 将 epT7 应用于四种不同 RNA 序列的环化，发现 epT7 生成的 circRNA 的翻译效率比野生型 T7 RNAP 生成的 RNA 高 3 到 30 倍（图 6B）。  

**凝胶电泳分析。** 使用 TapeStation 凝胶电泳 定量 IVT 后环状产物的相对比例，发现 epT7 生成的 circRNA 中长串联体（concatemer）的形成减少（图 6C）。为了进一步理解 epT7 生成 circRNA 翻译效率提高的机制，使用 2% E-gel EX 凝胶电泳分析 RNase R 处理前后的前体、切口、中间体和完整 circRNA 的相对比例（图 6D）。  

**circRNA 生产效率与纯度。** 观察到 epT7 生成的 circRNA 中中间体和切口副产物减少，表明转录保真度更高。通过凝胶电泳结果量化三种不同模板的 circRNA 比例，发现 epT7 的 circRNA 生产效率约为 25%，比野生型 T7 RNAP 高约 2 倍，且 circRNA 纯度更高，串联体生成更少（图 6E）。  

**dsRNA 含量分析。** 使用 dsRNA ELISA 检测 RNase R 清理后产物中的 dsRNA 含量。野生型 T7 RNAP 生成的 dsRNA 比例约为 1.5%，而 epT7 生成的 dsRNA 比例显著降低至 0.2%，表明 epT7 在长转录过程中具有更高的保真度（图 6F）。  

**circRNA 的稳定性验证。** 为了验证 circ-eGFP RNA 的更高稳定性，将野生型 T7 和 epT7 生成的 circRNA 转染到 HEK293FT 细胞 中，并在转染后 24 小时和 72 小时成像。观察到 epT7 生成的 circRNA 的 GFP 荧光强度高于野生型 T7 RNAP，且在 72 小时后仍保持稳定的 GFP 表达。  

![](EVOLVEpro蛋白质语言模型驱动的快速定向进化/EVOLVEpro蛋白质语言模型驱动的快速定向进化_2025-02-04-11-46-36.png)  

### 用于体内生物发光成像的 mRNA
epT7 具有高保真度，因此将其与野生型 T7 RNAP 在生成 100% N1-甲基假尿苷-5′-三磷酸修饰的萤火虫荧光素酶 mRNA 方面进行比较。这种修饰 mRNA 常用于体内深层组织成像（图 6G）。

将生成的 mRNA 包装到 脂质纳米颗粒（LNPs） 中，这些 LNPs 会靶向肝脏用于生物发光成像。注射 mRNA-LNPs 后 24 小时，观察到 epT7 生成的 mRNA 的发光强度比野生型 T7 RNAP 生成的 mRNA 高约 10 倍（图 6H）。进一步追踪两种 mRNA 的表达动力学，发现在 96 小时内，epT7 生成的萤火虫荧光素酶 mRNA 的翻译效率始终高于野生型 T7 RNAP 生成的 mRNA（图 6I）。  

### 探究 EVOLVEpro 进化中突变的新颖性
文中通过计算每个突变与训练输入（Uniprot）相比的突变可能性，分析了EVOLVEpro在本研究中进化的六种蛋白质上提出的突变。发现每种蛋白质的突变集合的中位突变可能性在0.01到0.04之间，远低于0.05的稀有突变阈值（图6J）。模型探索的大多数突变都是自然界中不常见的突变，定义为概率小于0.1的突变。其中，92%的PE2 MLV RT突变和77%的Bxb1整合酶突变低于这个阈值。此外，在本研究中六个蛋白质进化过程中探索的所有最佳活性增强单突变体的突变可能性都小于0.1。这一分析表明，与基于零样本的语言模型相比，EVOLVEpro探索的突变景观具有高度的原创性，并强调了在自然界发生的突变之外寻找活性增强突变体的必要性。  

## 结论
EVOLVEpro被证明是一种基于少量样本主动学习的蛋白质活性定向进化模型。经过多轮改进，EVOLVEpro生成的变体在目标特性（如结合能力、催化效率和免疫原性副产物）上实现了2到515倍的提升。通过结合进化尺度的蛋白质语言模型（PLMs）和回归层，EVOLVEpro能够学习蛋白质活性的一般规律，仅需少量进化周期即可生成高活性突变体。此外，由于PLM生成的丰富潜在空间和顶层模块的强大特征选择能力，EVOLVEpro是一种低样本需求的学习方法，仅需最少的湿实验验证。  

## 参考文献
Jiang K, Yan Z, Di Bernardo M, et al. Rapid in silico directed evolution by a protein language model with EVOLVEpro[J]. Science, 2024: eadr6006.  